...
const getBgColor = ()=>{
  ...
  return (
    ...
    <Typography sx={{display:"flex",justifyContent:"center" , alignItems:"center", height:50, textDecoration: task.completed ? 'line-through' : 'none'}}  variant='body2' color={'text.secondary'}>{task.summary}</Typography>
    </CardContent>
    <CardActions sx={{display:"flex", justifyContent:"end"}}>
<Box sx={{display:"flex",flexDirection:"row",gap:"20px",  }}>
  {title==="Backlog"&&<Tooltip   title="Move to WIP" arrow  placement='top' >
       <Fab size='small' variant='extended' color="primary" aria-label="moveTo"  onClick={() => {handleTaskMove(task.id)
                  } }>
   ...
   {title==="In Progress"&&<Tooltip   title="Move to Backlog" arrow  placement='top' >
       <Fab size='small' variant='extended' color="primary" aria-label="move"  onClick={() => {handleTaskMoveBack(task.id)
                  } }>
   ...
   {title==="In Progress"&&<Tooltip title="Complete" arrow  placement='top' >
       <Fab variant='extended' size='small' color="primary" aria-label="complete"  onClick={() => {handleTaskClick(task.id)
