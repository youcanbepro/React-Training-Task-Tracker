  ...
  return (
    <>
      <Grid key={task.task.id} container justifyContent={"space-between"}>
        <Grid
          style={{
            pointerEvents: task.task.completed === true ? "none" : "all",
            opacity: task.task.completed === true ? 0.6 : 1
          }}
          item
          {...useLongPress(() => sendMsg({ id: task.task.id, msgAc: msgAction.edit }), { ms: 1500 })}
        >
          <Typography
